...
test("conversation turns are automatically persisted as searchable summaries", async () => {
  ...
  try {
    ...
    await writer.handle({ type: "transcript.final", sessionId: "session-1", text: "Řešili jsme barge-in.", source: "input" });
    await writer.handle({ type: "transcript.partial", sessionId: "session-1", text: "Ano,", source: "output" });
    await writer.handle({ type: "transcript.partial", sessionId: "session-1", text: " opravili jsme ho.", source: "output" });
    await writer.handle({ type: "output.audio_completed", sessionId: "session-1", outputId: "output-1" });
